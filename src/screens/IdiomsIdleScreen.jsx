...
import idiomsData from '../data/idle/idleidioms.json'
...
function parseSegments(text) {
    ...
    while ((match = regex.exec(text)) !== null) {
        ...
        const seg = { text: content };

        if (tag.startsWith('h')) seg['highlight' + (tag[1] === '1' ? '' : tag[1])] = true;
        if (tag.startsWith('c')) seg['colorize' + (tag[1] === '1' ? '' : tag[1])] = true;
        if (tag.startsWith('b')) seg['badge' + (tag[1] === '1' ? '' : tag[1])] = true;
...
function parseIdiomText(rawStr) {
    if (!rawStr) return null;

    const colonIdx = rawStr.indexOf(':');
    const word = colonIdx > -1 ? rawStr.substring(0, colonIdx).trim() : rawStr;
    const rest = colonIdx > -1 ? rawStr.substring(colonIdx + 1).trim() : '';

    let arabicMeaning = '';
    let definitionStr = rest;
    let exampleStr = '';

    const b1Start = rest.indexOf('/b1');
    if (b1Start > -1) {
        definitionStr = rest.substring(0, b1Start).trim();
        const b1End = rest.indexOf('/b1', b1Start + 3);
        if (b1End > -1) {
            arabicMeaning = rest.substring(b1Start + 3, b1End).trim();
            const afterB1 = rest.substring(b1End + 3).trim();
            const exIdx = afterB1.indexOf('Example:');
            if (exIdx > -1) {
                exampleStr = afterB1.substring(exIdx + 8).trim();
            } else {
                exampleStr = afterB1;
            }
        }
        ...
        if (exIdx > -1) {
            definitionStr = rest.substring(0, exIdx).trim();
            exampleStr = rest.substring(exIdx + 8).trim();
        }
    }
    ...
    return {
        ...
        wordType: 'idiom',
        arabicMeaning,
        definition: parseSegments(definitionStr),
        example: parseSegments(exampleStr)
    };
}

const rawIdiomsArray = Array.isArray(idiomsData) ? idiomsData : (idiomsData.idioms || []);
const parsedIdiomsData = rawIdiomsArray.map(item => {
    if (typeof item === 'string') return parseIdiomText(item);
    return item;
}).filter(Boolean);
...
// MAIN IDLE SCREEN
// ─────────────────────────────────────────────
export function IdiomsIdleScreen({ onBack, color, onAccentChange }) {
    // Shuffled playlist
    const [playlist, setPlaylist] = useState(() => shuffleIndices(parsedIdiomsData.length))
    const [playlistIndex, setPlaylistIndex] = useState(0)
    const [phase, setPhase] = useState('front')  // 'front' | 'back' | 'exiting'
    const [isFlipped, setIsFlipped] = useState(false)
    const [isMouseIdle, setIsMouseIdle] = useState(false)

    const currentWord = parsedIdiomsData[playlist[playlistIndex]]
        ...
        setTimeout(() => {
            ...
            setPlaylistIndex(prev => {
                if (prev + 1 >= playlist.length) {
                    setPlaylist(shuffleIndices(parsedIdiomsData.length))
                    return 0
                }
                return prev + 1
            })
